...
//----------------------------------------------------------------------
//	Workaround for bug in JDK 1.1.5, fixed in 1.1.8
//----------------------------------------------------------------------
class BugFixObservable extends Observable {
	public void setChanged2()	{ setChanged(); }	// bug in JDK 1.1.5
}
 ...
 *	This widget is two interfaces to one float value.  The TextField
							...
							implements ActionListener, AdjustmentListener
{
	...
	BugFixObservable	_valueObservable;	// place holder, can't make the class Observable
	
	//----------------------------------------------------------------------
	//	Constructor
	/**
	 *	Specify the numerical range of this variable, this is independant of the
	 *	slider pixel values.
	 *	
	 ...
	 *
	 *	@see java.awt.Scrollbar#Scrollbar(int, int, int, int, int)
	 */
	//----------------------------------------------------------------------
	...
	{
		_valueObservable = new BugFixObservable();
		_min = inMinValue;
		_max = inMaxValue;
	...
	public void actionPerformed( ActionEvent inEvent )
	{
		if (inEvent.getSource() instanceof TextField ) {
			...
			System.err.println("Bad action recieved by ValueSlider.");
	}
	...
	public void adjustmentValueChanged( AdjustmentEvent inEvent )
	{
		if (inEvent.getSource() instanceof Scrollbar ) {
			...
			System.err.println("Bad adjustment recieved by ValueSlider.");
	}
